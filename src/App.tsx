...
const extractTextFromPdf = async (arrayBuffer: ArrayBuffer) => {
  ...
  for (let pageIndex = 1; pageIndex <= pdf.numPages; pageIndex += 1) {
    ...
    const pageText = rawText
      ...
      .replace(/\s{2,}/g, " ")
      .trim();
    sections.push({ title: `Page ${pageIndex}`, text: pageText });
  }
        ...
        <div className="app">
          <div className="reader">
            <div className="reader-frame">
              <div className="word" style={{ fontSize: "24px" }}>
  ...
  useEffect(() => {
    ...
    const frameId = requestAnimationFrame(() => {
      ...
      if (wordWidth > availableWidth) {
        setScale(Math.max(0.5, availableWidth / wordWidth));
      } else {
        setScale(1);
      }
    });
...
function Reader() {
  ...
  }, [currentIndex, isPlaying, wpm, words.length]);
